...
function App() {
  ...
  useEffect(async () => { await fetch('/dataArray.json').then((response) => response.json().then((dataItem) => setDataOrigin(dataItem))) }, []);
  ...
  const reverceArrayFun = (array) => {
    switch (tabIndex) {
      ...
      case 6:
        return array
      case 7:
        console.log("1111. " + moment().diff(new Date(dateFilterStart)), 'days')
        console.log("2222. " + moment().diff(new Date(array[200].date)), 'days')
        console.log("3333. " + new Date(array[200].date))
        console.log("3333. " + moment().diff(new Date(array[200].date)), 'days')
        console.log("..... ")
        console.log(moment().diff(new Date(array[200].date)), 'days')
  ...
  const option = {
    ...
    legend: {
      data: ['Shorts', 'Longs', 'BTC Price']
    },
    xAxis: [
      {
        type: 'category',
        data: dataOrigin?.btc_currency ? reverceArrayFun(dataOrigin.btc_currency).map(itemI =>
          (`${itemI.date.split("-")[2] + " " + months[Number(itemI.date.split("-")[1]) - 1] + " " + itemI.date.split("-")[0]}`)
        ) : [],
        axisPointer: {
          type: 'shadow'
        }
    ...
    series: [
      {
        ...
        type: 'bar',
        stack: 'one',
        data: dataOrigin?.btc_liquidation_short ? reverceArrayFun(dataOrigin.btc_liquidation_short).map(itemI => -itemI.value) : [],
      },
      {
        name: 'Longs',
        type: 'bar',
        stack: 'one',
        data: dataOrigin?.btc_liquidation_long ? reverceArrayFun(dataOrigin.btc_liquidation_long).map(itemI => itemI.value) : [],
      },
      {
        name: 'BTC Price',
        type: 'line',
        yAxisIndex: 1,
        showSymbol: false,
        data: dataOrigin?.btc_currency ? reverceArrayFun(dataOrigin.btc_currency).map(itemI => Number(itemI.value)) : [],
      }
    ]
  };
  ...
  return (
    <div className='containerCal'>
      <div className='wrapFilters'>
        <Tabs selectedIndex={tabIndex} onSelect={index => { if (index !== 7) { setTabIndex(index) } else { setTabIndex(index); setOpenModal(true); } }}>
          <TabList>
            <Tab>7D</Tab>
            <Tab>1M</Tab>
            <Tab>3M</Tab>
